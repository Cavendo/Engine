-- Migration 001: Add encryption key version columns (PostgreSQL)
-- Tracks which keyring version was used to encrypt each credential

ALTER TABLE agents ADD COLUMN IF NOT EXISTS encryption_key_version INTEGER DEFAULT NULL;

ALTER TABLE storage_connections ADD COLUMN IF NOT EXISTS access_key_id_key_version INTEGER DEFAULT NULL;
ALTER TABLE storage_connections ADD COLUMN IF NOT EXISTS secret_access_key_key_version INTEGER DEFAULT NULL;
