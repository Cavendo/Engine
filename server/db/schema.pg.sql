...
CREATE UNIQUE INDEX IF NOT EXISTS idx_runtime_skill_policies_unique_ws
  ON runtime_skill_policies(skill_key, role, workspace_id) WHERE workspace_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_runtime_skill_policies_unique_global
  ON runtime_skill_policies(skill_key, role) WHERE workspace_id IS NULL;
